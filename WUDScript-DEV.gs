...
function syncCalendarDEV() { 
  function convertDateToUTC(date) { return new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds()); }
  var sheetTabName = "2 Weeks"; // Put the tab name here
  var calendarName = "GPNJ - What's Up Doc Calendar-DEV"; // Put the Calendar name here
  ...
  while( index < numRows)
  {
    ...
    if(curDateValue != null || isValidDate(dateOnCell)){
      ...
      // Format start and end dates if it's weird looking
...
function clearAllEvents() {
  var myCalendar = CalendarApp.getCalendarsByName("GPNJ - What's Up Doc Calendar-DEV")[0];
  var now = new Date();
